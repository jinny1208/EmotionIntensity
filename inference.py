...
def load_face_image(face_path: str, target_size: int = 160) -> torch.Tensor:
    """
    Aspect-ratio-preserving resize + center crop → (1, 3, H, W) float [0,255]
    """
    transform = transforms.Compose([
        transforms.Resize(target_size),        # shorter side → target_size
        transforms.CenterCrop(target_size),    # square crop, no distortion
        transforms.ToTensor(),                 # (3, H, W) in [0, 1]
        transforms.Lambda(lambda x: x * 255.0),
    ])
    img = Image.open(face_path).convert("RGB")
    return transform(img).unsqueeze(0)         # (1, 3, 160, 160)
...
def main():
    ...
    for sample in tqdm(samples, desc="Generating"):
        ...
        out_name = f"{lineno:05d}_{emotion}_{intensity}.wav"
        ...
        try:
            ...
            # face image
            face = load_face_image(image_path).to(device)
