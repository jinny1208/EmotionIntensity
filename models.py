...
class ReferenceEncoder(nn.Module):
    ...
    def forward(self, inputs, mask=None):
        ...
        return out.squeeze(0) #out.shape(1, 32, 128) --> squeeze(32,128)
...
class KernelPredictor(nn.Module):
    def __init__(self,
        ...
        #* (w_depthwise, b_depthwise, gain_depthwise, w_pointwise, b_pointwise)
    ...
    def forward(self, x):
        #* x (style vector): (B, 128)
        batch = x.size(0)
        x = self.proj(x)
        d_w, d_g, d_b, p_w, p_g, p_b = torch.split(x,
            [self.d_w_ch, self.d_g_ch, self.d_b_ch, self.p_w_ch, self.p_g_ch, self.p_b_ch], dim=1)
...
class TextEncoder(nn.Module):
  def __init__(self,
    ...
    self.kernel_predictor = KernelPredictor(gin_channels, n_layers, sc_ker_size, sc_ch_size, sc_ch_size)
  ...
  def forward(self, x, x_lengths, g):
    ...
    # g.shape: torch.Size([32, 128, 1])
     
    sc_kernels = self.kernel_predictor(g.squeeze(-1)) # (6개의 kernel variables)
    x = self.emb(x) * math.sqrt(self.hidden_channels) # [b, t, h]
    x = torch.transpose(x, 1, -1) # [b, h, t]
    x_mask = torch.unsqueeze(commons.sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype)

    x, x_i = self.encoder(x * x_mask, x_mask, sc_kernels)
    stats = self.proj(x) * x_mask

    m, logs = torch.split(stats, self.out_channels, dim=1)
    return x, x_i, m, logs, x_mask
...
class SynthesizerTrn(nn.Module):
  ...
  def forward(self, x, x_lengths, y, y_lengths, face, sid=None):
    ...
    w = attn.sum(2)
    if self.use_sdp:
      l_length = self.dp(x_i, x_mask, w, g=g)
      l_length = l_length / torch.sum(x_mask)
    else:
      logw_ = torch.log(w + 1e-6) * x_mask
      logw = self.dp(x_i, x_mask, g=g)
  ...
  def infer(self, x, x_lengths, face, sid=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
    ...
    if self.use_sdp:
      logw = self.dp(x_i, x_mask, g=g, reverse=True, noise_scale=noise_scale_w)
    else:
      logw = self.dp(x_i, x_mask, g=g)
